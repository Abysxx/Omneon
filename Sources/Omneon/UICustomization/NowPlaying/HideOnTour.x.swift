...
struct HideOnTour: HookGroup {}

var hiddenIndexPath_5: IndexPath? = nil

func scanAndHideOnTour(in collectionView: UICollectionView) {
    for cell in collectionView.visibleCells {
        if containsIdentifier(cell, identifier: "Components.UI.OnTourEventCard"),
           let indexPath = collectionView.indexPath(for: cell) {
            if hiddenIndexPath_5 != indexPath {
                hiddenIndexPath_5 = indexPath
...
class HideOnTour_ViewControllerHook: ClassHook<UIViewController> {
    typealias Group = HideOnTour
    ...
    func nowPlayingScrollViewModelWithDidLoadComponents(for arg1: AnyObject, withDifferentProviders arg2: Bool, scrollEnabledValueChanged arg3: Bool) {
        ...
        scanAndHideOnTour(in: collectionView)
    }
    ...
    func nowPlayingScrollViewModelWithDidMoveToRelativeTrack(for arg1: AnyObject, withDifferentProviders arg2: Bool, scrollEnabledValueChanged arg3: Bool) {
        ...
        hiddenIndexPath_5 = nil
        scanAndHideOnTour(in: collectionView)
    }
}

class HideOnTour_DelegateHook: ClassHook<NSObject> {
    typealias Group = HideOnTour
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        let adjusted = hiddenIndexPath_5 != nil ? count - 1 : count
        return adjusted
    }

    @objc(collectionView:cellForItemAtIndexPath:)
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let hidden = hiddenIndexPath_5 else {
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        if hiddenIndexPath_5 == nil && containsIdentifier(cell, identifier: "Components.UI.OnTourEventCard") {
            hiddenIndexPath_5 = indexPath
            DispatchQueue.main.async {
                collectionView.reloadData()
            }
